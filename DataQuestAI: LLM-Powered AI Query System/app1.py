...
# Streamlit App
def main():
    st.set_page_config(page_title="Flexible RAG System", page_icon="🤖", layout="centered")
    st.title("Flexible RAG System 🤖")
    ...
    # root_dir = st.sidebar.text_inputpip("Enter the root directory path:")
    root_dir = st.sidebar.text_input("Enter the root directory path:", "C:/Users/hp/Desktop/RAG/data")
    if not root_dir:
    ...
    if root_dir and st.sidebar.button("Process Documents"):
        try:
            ...
            # Generate embeddings
            embeddings = OpenAIEmbeddings() if llm_type == "OpenAI" else HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
